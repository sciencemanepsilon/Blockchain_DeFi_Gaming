...
cache = TTLCache(500, cacheSeconds)
...
def cacheSetter():
    ...
    with lock:
        ...
        print(f"batch {ii} start")
        batchStart = time.time()
        for lid in cacheArray.keys():
            finalUrl = cacheUrl.format(cacheArray[lid], dateNow, dateEnd)
            if ii in {5, 10, 15, 20, 25, 30, 35, 40, 45, 50}:
                tp = time.time() - batchStart
                print(f"call {ii} batchTime {tp}")
                if tp < 1.08: time.sleep(1 -tp)
                batchStart = time.time()
            try:
                res = sess.post(
                    finalUrl,
