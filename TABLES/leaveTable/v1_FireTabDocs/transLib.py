...
with Client(timeout=None) as syncHpx:
    data = syncHpx.get(
        environ['SYS_CONFIG_API'], params={"oriApi":"LeaveTable"},
        headers={"Authorization":f"Bearer {getJWT(environ['SYS_CONFIG_API'])}"}
    ).json()

collNames, beta, live, blockApi, oriWhiteList, topicPath = (
    data['Transes']['FromOri'],
    data['beta'], data['live'], data['blockApi'],
    data['oriWhiteList'], data['LeaveTableTopicPath']
)
...
asyncHpx = {
    "age":time(), "hpx":None, "token":getJWT(blockApi)
}

async def callContract(tid, plArr, tries, nnew):
    ...
    headers = setNewHttpxClient(headers, nnew)
    try:
        resp = await asyncHpx['hpx'].post(
            blockApi +"/ContractFunc", headers=headers,
            json={"funcName":"leaveGame", "tid":tid, "playersArr":plArr}
        )
        print(f"contractRes: {resp.status_code} headers {len(headers)}")
    except Exception as e:
        print(f"tries {tries}, contract call failed: {e}")
        if str(e) == "Event loop is closed": return -9
        return 505
    if resp.status_code != 200:
        print(f"contract response {resp.status_code} tries {tries}")
        return resp.status_code
    return resp.status_code


def setNewHttpxClient(headers, nnew):
    if not asyncHpx['hpx']:
        ...
        asyncHpx['hpx'] = AsyncClient(timeout=None)
        print("setting new httpx client without header")
