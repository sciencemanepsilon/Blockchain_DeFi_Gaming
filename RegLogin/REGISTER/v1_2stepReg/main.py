...
def register():
    ...
    errMsg = validateNick(nick)
    if errMsg:
        print(errMsg)
        return jsonify({"error":errMsg}), 200
    ...
    # Create Auth Object
    user, errMsg = setAuthObject(nick)
    ...
    erg = EsIndexNickWriteSessBQ(
        user.uid, nick, ipAdd, isMobile,
        device, countryCode, email, wid
    )
    if erg['error']: return jsonify({"error":erg["error"]}), 200
    ...
    return jsonify({"error":False, "idtoken":id_token}), 200
